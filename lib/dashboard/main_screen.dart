...
import 'package:jolly_podcast/dashboard/page/libary_page.dart';
import 'package:jolly_podcast/utils_folder/app_colors.dart';
import 'package:jolly_podcast/utils_folder/custom_appbar.dart';
import 'package:jolly_podcast/utils_folder/custom_shimmer.dart';
...
class _MainScreenViewState extends State<_MainScreenView> {
  int _index = 0;
  final pages = const [DiscoverPage(), CategoryPage(), LibaryPage()];

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: AppColors.color13,
      appBar: const JollyAppBar(),
      body: BlocBuilder<EpisodeCubit, EpisodeState>(
        builder: (context, state) {
          if (state is EpisodeLoading) {
            return const DiscoverLikeShimmer(); // same shimmer layout
          }
